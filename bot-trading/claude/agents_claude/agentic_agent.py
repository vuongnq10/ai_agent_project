...
from claude.agents_claude.agent import Agent
...
class MasterClaude:
    ...
    def _call_master(self, state: MasterState) -> MasterState:
        ...
        response = agent(state["chat_history"], system=_MASTER_SYSTEM_INSTRUCTION)
    ...
    def _tool_agent(self, state: MasterState) -> MasterState:
        try:
            ...
            response = agent(state["chat_history"], tools=_CLAUDE_TOOLS)
