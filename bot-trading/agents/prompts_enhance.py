...
for THREE timeframes (1h, 2h, 4h) sent directly from the frontend.
...
You receive pre-computed SMC indicators for THREE timeframes (1h, 2h, 4h).
...
STEP 3 — 1H EXECUTION (entry trigger)
  Read the 1h data:
  - Has the 1h last_choch direction fired in the bias direction?
  - Is the 1h current_price at or near the 2h POI zone?
  - EMA alignment on 1h: ema9 > ema20 > ema50 for buys; ema9 < ema20 < ema50 for sells
  - RSI14 on 1h: < 65 for buys (not overbought), > 35 for sells (not oversold)
...
Score the setup (0–19 points):
  ...
  +2 if 1h CHoCH direction matches bias
  +1 if liquidity pool exists on the target side
  +1 if 1h RSI supports direction (< 40 for buy, > 60 for sell)
  +1 if 1h EMA stack fully aligned with direction
...
Respond in JSON format:
{
    ...
    "stop_loss": "29.00",
    "take_profit": "32.00",
    "reasoning": "4h bias + 2h POI + 1h trigger summary",
    ... additional relevant details ...
}
"""
...
You receive the multi-timeframe analysis (4h bias, 2h POI, 1h trigger) and must
...
--- BUY requires all of the following ---
  ...
  Gate 4 (1h Trigger): 1h last_choch = "bullish" while 1h price is AT or BELOW the 2h POI zone
...
--- SELL requires all of the following ---
  ...
  Gate 3 (2h POI):     unmitigated bearish OB (strength >= 50) AND unfilled bearish
                       FVG (strength >= 30) on 2h, confluence_score >= 50
  Gate 4 (1h Trigger): 1h last_choch = "bearish" while 1h price is AT or ABOVE the 2h POI zone
  Gate 5 (R:R):        take_profit distance >= 1.5x stop_loss distance
...
--- WAIT when any of the following is true ---
  ...
  - 1h RSI14 > 70 when considering a BUY, or < 30 when considering a SELL
  - 1h EMA stack opposes the bias direction
