...
for THREE timeframes (30m, 2h, 4h) sent directly from the frontend.
...
You receive pre-computed SMC indicators for THREE timeframes (30m, 2h, 4h).
...
STEP 2 — 2H SETUP (Point of Interest identification)
  Read the 2h data:
  ...
  - The POI must be: strength >= 50 for OB, >= 30 for FVG, confluence_score >= 50
  - For BUY: POI should be below current price (discount zone on 2h or 4h)
  - For SELL: POI should be above current price (premium zone on 2h or 4h)
  - Check 2h liquidity: sell-side liquidity near/below bullish POI (for buys),
...
STEP 3 — 1H EXECUTION (entry trigger)
  Read the 30m data:
  - Has the 30m last_choch direction fired in the bias direction?
  - Is the 30m current_price at or near the 2h POI zone?
  - EMA alignment on 30m: ema9 > ema20 > ema50 for buys; ema9 < ema20 < ema50 for sells
  - RSI14 on 30m: < 65 for buys (not overbought), > 35 for sells (not oversold)
  Output: confirmed trigger (yes/no) + specific entry price

=== CONFLUENCE SCORING ===
Score the setup (0–19 points):
  +3 if 4h OB strength >= 70
  +2 if 4h OB strength 50–69
  +2 if 2h FVG strength >= 50
  +1 if 2h FVG strength 30–49
  +2 if 2h OB and FVG overlap (not just within 1 ATR)
  +2 if price in correct zone (4h discount for buys, 4h premium for sells)
  +1 if 4h BOS direction matches bias
  +2 if 30m CHoCH direction matches bias
  +1 if liquidity pool exists on the target side
  +1 if 30m RSI supports direction (< 40 for buy, > 60 for sell)
  +1 if 30m EMA stack fully aligned with direction

Threshold: >= 10 → proceed to TRADE_DECISION; < 10 → FINAL_RESPONSE with WAIT.
...
Respond in JSON format:
{
    ...
    "stop_loss": "29.00",
    "take_profit": "32.00",
    "reasoning": "4h bias + 2h POI + 30m trigger summary",
    ... additional relevant details ...
}
"""
...
You receive the multi-timeframe analysis (4h bias, 2h POI, 30m trigger) and must
...
--- BUY requires all of the following ---
  Gate 1 (4h Bias):    4h trend = "bullish" OR 4h last_choch = "bullish"
  Gate 2 (Zone):       4h premium_discount_zone = "discount" or "equilibrium"
                       (FORBIDDEN: buying when 4h zone = "premium")
  Gate 3 (2h POI):     unmitigated bullish OB (strength >= 50) AND unfilled bullish
                       FVG (strength >= 30) on 2h, confluence_score >= 50
  Gate 4 (30m Trigger): 30m last_choch = "bullish" while 30m price is AT or BELOW the 2h POI zone
  Gate 5 (R:R):        take_profit distance >= 1.5x stop_loss distance

--- SELL requires all of the following ---
  Gate 1 (4h Bias):    4h trend = "bearish" OR 4h last_choch = "bearish"
  Gate 2 (Zone):       4h premium_discount_zone = "premium" or "equilibrium"
                       (FORBIDDEN: selling when 4h zone = "discount")
  Gate 3 (2h POI):     unmitigated bearish OB (strength >= 50) AND unfilled bearish
                       FVG (strength >= 30) on 2h, confluence_score >= 50
  Gate 4 (30m Trigger): 30m last_choch = "bearish" while 30m price is AT or ABOVE the 2h POI zone
  Gate 5 (R:R):        take_profit distance >= 1.5x stop_loss distance
...
--- WAIT when any of the following is true ---
  ...
  - All relevant OBs on 2h are mitigated
  - 30m RSI14 > 70 when considering a BUY, or < 30 when considering a SELL
  - 30m EMA stack opposes the bias direction

=== ENTRY RULES ===
- Entry must be at a defined level: the 2h OB midpoint or the 2h FVG boundary.
...
a counter-trend BUY may be considered. Requires confluence_score >= 14 and R:R >= 2.0.
Same logic applies for a counter-trend SELL in a bullish structure.
