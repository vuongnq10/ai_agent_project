...
def _fetch_candles(symbol: str, timeframe: str, limit: int = 300) -> list[dict]:
    """Fetch OHLCV klines from Binance Futures REST API."""
...
def _find_swings(candles: list[dict], lookback: int = 5) -> tuple[list[dict], list[dict]]:
    """Detect swing highs and lows with a given lookback window."""
    highs, lows = [], []
...
def _score_ob(candle: dict, atr: float) -> int:
    """Score order block strength 0–100."""
...
def _score_fvg(high: float, low: float, atr: float) -> int:
    """Score fair value gap strength 0–100."""
    if atr == 0:
        return 0
...
def _calc_atr(candles: list[dict], period: int = 14) -> float:
    """Wilder's ATR (matches the TS implementation)."""
...
def _calc_bb(closes: list[float], period: int = 20, mult: float = 2.0) -> dict:
    """Bollinger Bands."""
    upper, middle, lower = [], [], []
...
def _calc_smc(candles: list[dict]) -> dict:
    """
    Full SMC analysis — direct Python port of fe_chat/src/App/indicators.ts calcSMC().
    Returns a dict with trend, swings, BOS, CHoCH, order blocks, FVGs,
    premium/discount zone, liquidity levels, and potential entries.
    """
    if len(candles) < 30:
    ...
    swing_highs, swing_lows = _find_swings(candles, 5)
    ...
    # Bullish OB: last red candle before swing low
    for i in range(len(swing_lows) - 1, max(-1, len(swing_lows) - 5), -1):
        ...
        for j in range(sw_idx, max(-1, sw_idx - 11), -1):
            if candles[j]["close"] < candles[j]["open"]:
                ...
                mitigated = any(
                    ob_low <= c["close"] <= ob_high for c in candles[j + 1:]
                )
                strength = 0 if mitigated else _score_ob(candles[j], atr)
                order_blocks.append({
    ...
    # Bearish OB: last green candle before swing high
    ...
    for i in range(1, len(candles) - 1):
        ...
        if prev["high"] < nxt["low"]:
            ...
            strength = 0 if filled else _score_fvg(gap_top, gap_bot, atr)
    ...
    # ── Liquidity ─────────────────────────────────────────────────────────────
    buy_side_liquidity = sorted(
        [s["price"] for s in swing_highs[-5:]], reverse=True
    )
    sell_side_liquidity = sorted(
        [s["price"] for s in swing_lows[-5:]]
    )

    # ── Potential Entries (OB + FVG confluence) ───────────────────────────────
    potential_entries = []
...
def _calc_classic_indicators(candles: list[dict]) -> dict:
    """Compute EMA (9/20/50), BB (20,2), RSI (7/14/21), ATR (14)."""
    ...
    bb = _calc_bb(closes, 20, 2.0)
    rsi7 = _calc_rsi(closes, 7)
    rsi14 = _calc_rsi(closes, 14)
    rsi21 = _calc_rsi(closes, 21)
...
class CXConnector:
    ...
    def smc_analysis(self, symbol: str, timeframe: str = "1h", limit: int = 200) -> dict:
        """
        Fetch OHLCV from Binance and compute full SMC + classic indicators.
        Returns a structured dict ready to be passed to the AI agent.
        """
        try:
            candles = _fetch_candles(symbol, timeframe, limit=limit)

            smc = _calc_smc(candles)
            indicators = _calc_classic_indicators(candles)
            current_price = candles[-1]["close"]

            # Trim swing arrays to last 10 for readability
            result = {
                "symbol": symbol,
                "timeframe": timeframe,
                ...
                "order_blocks": [
                    ob for ob in smc["order_blocks"] if not ob["mitigated"]
                ],
                ...
                # Classic indicators (latest value)
                **indicators,
            }
    ...
    def get_ticker(self, symbol: str, timeframe: str = "1h"):
        candles = _fetch_candles(symbol, timeframe, limit=100)
